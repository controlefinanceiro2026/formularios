...
async function carregarFormularios() {
    ...
    if (eP || eV) { tbody.innerHTML = linhaVazia(9, 'Erro ao carregar formulários.'); return; }

    const linhasPessoal = (pessoal || []).map(f => `
        <tr>
            <td>Pessoal</td>
    ...
    const linhasVeiculo = (veiculo || []).map(f => `
        <tr>
            <td>Veículo</td>
    ...
    tbody.innerHTML = linhas.length ? linhas.join('') : linhaVazia(9, 'Nenhum formulário recebido.');
}
...
async function carregarPessoal() {
    ...
    cachePessoal = data || [];
    if (!cachePessoal.length) { tbody.innerHTML = linhaVazia(11, 'Nenhuma pessoa cadastrada.'); return; }
...
async function carregarVeiculos() {
    ...
    cacheVeiculos = data || [];
    if (!cacheVeiculos.length) { tbody.innerHTML = linhaVazia(8, 'Nenhum veículo cadastrado.'); return; }
